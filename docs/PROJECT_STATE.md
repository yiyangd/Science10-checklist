...
- `codex/project-operations-baseline`: Stage 30A documentation branch, based on the verified `main` SHA above.
...
1. **Unresolved July 8 stash.** `stash@{0}` is named `On main: WIP Unit 3 distractor edits before Chapter 1.3 audit`. It has not been applied, popped, dropped, renamed, or reviewed in this stage. It requires a separate read-only audit before any decision.
...
The GitHub CLI was not installed in the Stage 30A workstation environment. Repository refs were verified with `git`, production with an HTTP probe, and branch workflow state should be recorded through an authenticated GitHub/API surface. This is an environment limitation, not an application defect.

## Next recommended stages

1. **Stage 30B: read-only legacy-stash audit.** Inventory the July 8 stash against its base and current `main`; do not mutate the stash or working tree.
2. **Canonical data and Quiz-quality validation.** Define evidence and human-review criteria beyond hash equality, then audit in small, traceable batches.
...
- The contents and present-day value of the July 8 stash are intentionally unknown until Stage 30B.
